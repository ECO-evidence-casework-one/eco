...
- The current `main` Windows PowerShell build script does not reliably fail on every native command exit code. A green Windows job must not be treated as proof that every Windows test command was enforced.
- P0 issue #24 found that the public Actions workflow distributed unsigned executable packages, including builds generated by documentation-only pull requests. New uploads were stopped in `bdc05df444d21d739abf83fa9cf768fc4ab5dd9a`, but four historical executable artifacts remain unapproved until deleted or expired and exact no-artifact runs are evidenced.
