...
The following files preserve the records that were previously stored under generic root filenames:
...
Later commits changed the tracked tree, tests, workflow and public control documents. The historical manifest therefore cannot be used to verify a later commit.

## Current authority

There is no approved current release manifest or release receipt.
...
The generic root files `REPOSITORY_MANIFEST.csv` and `REPOSITORY_PREPARATION_RECEIPT.json` are retained only as explicit pointers to these historical records. They are not current integrity evidence.
