# ECO rig AI preview preparer v2.
...
function Assert-Sha([string]$Path,[string]$Expected) {
    $actual = Sha $Path
    if ($actual -cne $Expected) { throw "SHA-256 mismatch for $([IO.Path]::GetFileName($Path)): $actual" }
    $actual
}
...
function Run([string]$Label,[string]$Exe,[string[]]$Args) {
    & $Exe @Args | Out-Host
...
function Self-Test {
    $root=Join-Path ([IO.Path]::GetTempPath()) ('eco-ai-v2-'+[guid]::NewGuid().ToString('N'));[void][IO.Directory]::CreateDirectory($root)
    try{
        $f=Join-Path $root 'x';Set-Content -LiteralPath $f -Value 'x' -NoNewline -Encoding ascii;$h=Sha $f;[void](Assert-Sha $f $h)
        $rejected=$false;try{[void](Assert-Sha $f ('0'*64))}catch{$rejected=$true};if(-not $rejected){throw 'Hash rejection self-test failed.'}
        Write-Host 'Rig AI preparer v2 self-test PASS.'
    }finally{Remove-Item -LiteralPath $root -Recurse -Force -ErrorAction SilentlyContinue}
}
function Build-Eco([string]$Work) {
    $sourceZip=Join-Path $Work 'eco-source.zip';$goZip=Join-Path $Work 'go.zip'
    ...
    $src=Join-Path $Work 'source';$compiler=Join-Path $Work 'compiler'
    Expand-Checked $sourceZip $src;Expand-Checked $goZip $compiler
    $sourceRoots=@(Get-ChildItem -LiteralPath $src -Directory)
    if($sourceRoots.Count-ne 1){throw 'Unexpected ECO source archive layout.'};$src=$sourceRoots[0].FullName
    $portable=Find-PortableGo $compiler;$go=$portable.Go
    $names=@('GOENV','GOTOOLCHAIN','GOWORK','GOTELEMETRY','GOROOT','GOPATH','GOCACHE','GOMODCACHE','GOTMPDIR','GOOS','GOARCH','GOAMD64','CGO_ENABLED','GOMAXPROCS','GOFLAGS','GOEXPERIMENT','GODEBUG','GOPROXY','GOSUMDB','GOPRIVATE','GONOSUMDB','GONOPROXY','TEMP','TMP');$saved=@{};foreach($n in $names){$saved[$n]=[Environment]::GetEnvironmentVariable($n,'Process')};$old=Get-Location
    try{
        $env:GOROOT=$portable.Root;$env:GOENV='off';$env:GOTOOLCHAIN='local';$env:GOWORK='off';$env:GOTELEMETRY='off';$env:GOPATH=Join-Path $Work 'cache';$env:GOCACHE=Join-Path $Work 'cache\build';$env:GOMODCACHE=Join-Path $Work 'cache\modules';$env:GOTMPDIR=Join-Path $Work 'tmp';[void][IO.Directory]::CreateDirectory($env:GOTMPDIR);$env:TEMP=$env:GOTMPDIR;$env:TMP=$env:GOTMPDIR;$env:GOOS='windows';$env:GOARCH='amd64';$env:GOAMD64='v1';$env:CGO_ENABLED='0';$env:GOMAXPROCS='2';$env:GOFLAGS='-mod=readonly -buildvcs=false';$env:GOEXPERIMENT='';$env:GODEBUG='';$env:GOPRIVATE='';$env:GONOSUMDB='';$env:GONOPROXY='';$env:GOPROXY='https://proxy.golang.org';$env:GOSUMDB='sum.golang.org'
        ...
        Write-Host '[3/7] Verifying dependencies.';Run 'go mod download' $go @('mod','download');Run 'go mod verify' $go @('mod','verify');$env:GOPROXY='off'
        Write-Host '[4/7] Running ECO tests and vet.';Run 'go test' $go @('test','-count=1','-p=2','./...');Run 'go vet' $go @('vet','-p=2','./...')
        Write-Host '[5/7] Reproducing the qualified ECO executable twice.';$a=Join-Path $Work 'ECO.exe';$b=Join-Path $Work 'ECO.rebuild.exe';$flags="-s -w -H windowsgui -buildid= -X github.com/ECO-evidence-casework-one/eco/internal/eco.SourceCommit=$EcoSource";Run 'ECO build 1' $go @('build','-trimpath','-buildvcs=false','-ldflags',$flags,'-o',$a,'./cmd/eco');Run 'ECO build 2' $go @('build','-trimpath','-buildvcs=false','-ldflags',$flags,'-o',$b,'./cmd/eco');[void](Assert-Sha $a $EcoExeSHA);[void](Assert-Sha $b $EcoExeSHA);if((Get-Item $a).Length-ne $EcoExeSize){throw 'ECO size mismatch.'};Remove-Item $b -Force
...
function Install-Llama([string]$Work,[string]$Dest) {
    Write-Host '[6/7] Downloading hash-pinned llama.cpp b10259 from GitHub.';$zip=Join-Path $Work 'llama.zip';Download-Checked $LlamaURL $zip $LlamaZipSHA;$tmp=Join-Path $Work 'llama';Expand-Checked $zip $tmp;$llama=Find-Llama $tmp;Move-Item -LiteralPath $tmp -Destination $Dest;$llama=Find-Llama $Dest;$version=(& $llama '--offline' '--version' 2>&1|Out-String).Trim();if($LASTEXITCODE-ne 0-or-not $version){throw 'llama.cpp version probe failed.'};[pscustomobject]@{Exe=$llama;SHA=(Sha $llama);Version=($version-split "`r?`n")[0]}
}
function Get-Model([string]$Dest) {
    [void][IO.Directory]::CreateDirectory($Dest);$p=Join-Path $Dest $ModelName
    if(Test-Path $p){[void](Assert-Sha $p $ModelSHA);return $p}
    Write-Host '[7/7] Downloading official Qwen2.5 1.5B Q4_K_M and verifying its published SHA-256.';Download-Checked $ModelURL $p $ModelSHA;$p
}
function Probe-Qwen([string]$Llama,[string]$Model,[string]$Work) {
    $p=Join-Path $Work 'probe.txt';$s=Join-Path $Work 'schema.json';Set-Content $p 'Return only the JSON object required by the schema. Set ok to ECO_AI_READY.' -NoNewline -Encoding utf8;Set-Content $s '{"type":"object","additionalProperties":false,"required":["ok"],"properties":{"ok":{"type":"string","enum":["ECO_AI_READY"]}}}' -NoNewline -Encoding utf8
    $out=(& $Llama '--offline' '--model' $Model '--file' $p '--json-schema-file' $s '--simple-io' '--no-display-prompt' '--color' 'off' '--log-disable' '--seed' '0' '--temp' '0' '--top-k' '1' '--top-p' '1' '--min-p' '0' '--ctx-size' '2048' '--n-predict' '64' '--device' 'none' '--n-gpu-layers' '0' '--fit' 'off' '--no-context-shift' '--no-perf' 2>&1|Out-String).Trim();if($LASTEXITCODE-ne 0){throw "Qwen smoke test failed: $out"};try{$j=$out|ConvertFrom-Json}catch{throw "Qwen smoke test returned non-JSON: $out"};if($j.ok-cne 'ECO_AI_READY'){throw "Unexpected Qwen result: $out"};$out
}
function Write-Launcher([string]$Root,[string]$Llama,[string]$LlamaSHA) {
    $rel=$Llama.Substring($Root.TrimEnd('\').Length+1);@('@echo off','setlocal','cd /d "%~dp0"','if not exist "PreviewUserData" mkdir "PreviewUserData"','set "LOCALAPPDATA=%~dp0PreviewUserData"',"set `"ECO_LLAMA_CPP=%~dp0$rel`"","set `"ECO_LLAMA_CPP_SHA256=$LlamaSHA`"","set `"ECO_LLAMA_MODEL=%~dp0AIAssets\$ModelName`"","set `"ECO_LLAMA_MODEL_SHA256=$ModelSHA`"",'start "" "%~dp0ECO.exe"','endlocal')|Set-Content (Join-Path $Root 'START_ECO_WITH_AI.cmd') -Encoding ascii
}
