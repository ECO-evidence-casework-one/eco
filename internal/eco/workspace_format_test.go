...
func workspaceFormatFixture(t *testing.T) (string, []byte, map[string]json.RawMessage) {
	...
	if _, err := v.CreateMatter("Synthetic retained matter"); err != nil {
		_ = v.Close(); t.Fatal(err)
	}
	v.Workspace.Settings.LowSensory = true
	if err := v.Save(); err != nil { _ = v.Close(); t.Fatal(err) }
...
func workspaceFormatWrite(t *testing.T, root string, key, plain []byte) {
	...
	if err != nil { t.Fatal(err) }
	if err := os.WriteFile(filepath.Join(root, "workspace.ecodb"), data, 0600); err != nil { t.Fatal(err) }
}
	...
	for _, name := range cases {
		t.Run(name, func(t *testing.T) {
			...
			case "unknown_matter":
				...
				if err := json.Unmarshal(document["matters"], &matters); err != nil { t.Fatal(err) }
			...
			if name == "trailing_json" { plain = append(plain, []byte(` {"schema":1}`)...) }
			if name == "malformed_json" { plain = []byte(`{"schema":`) }
			workspaceFormatWrite(t, root, key, plain)
			switch name {
			case "authentication_failure":
				if err := os.WriteFile(filepath.Join(root, "workspace.ecodb"), []byte("not authenticated metadata"), 0600); err != nil { t.Fatal(err) }
			case "future_missing_objects":
				if err := os.Remove(filepath.Join(root, "objects")); err != nil { t.Fatal(err) }
			case "missing_key":
				if err := os.Remove(filepath.Join(root, "vault.key")); err != nil { t.Fatal(err) }
			case "missing_metadata":
				if err := os.Remove(filepath.Join(root, "workspace.ecodb")); err != nil { t.Fatal(err) }
			case "objects_only":
				for _, file := range []string{"vault.key", "workspace.ecodb"} {
					if err := os.Remove(filepath.Join(root, file)); err != nil { t.Fatal(err) }
				}
			case "unknown_without_owner_file":
				if err := os.Remove(filepath.Join(root, workspaceOwnerFilename)); err != nil && !os.IsNotExist(err) { t.Fatal(err) }
			}
			before := workspaceFormatTree(t, root)
			opened, err := OpenVault(root)
			if opened != nil {
				if closeErr := opened.Close(); closeErr != nil { t.Fatal(closeErr) }
			}
			if err == nil { t.Error("unsupported or incomplete workspace accepted for writing") }
...
func TestWorkspaceFormatLegacyDefaultsPreserved(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	defer v.Close()
	if !reflect.DeepEqual(before, workspaceFormatTree(t, root)) { t.Fatal("opening supported legacy state rewrote it") }
	ws := v.Snapshot()
...
func workspaceFormatBackup(t *testing.T, path, passphrase string, plain []byte) {
	...
	for _, n := range []uint32{backupVersion, iterations} {
		if err := binary.Write(&output, binary.LittleEndian, n); err != nil { t.Fatal(err) }
	}
	output.Write(salt)
	output.Write(prefix)
	if err := writeShortString(&output, BuildID); err != nil { t.Fatal(err) }
	digest := sha256.Sum256(plain)
	if err := writeBackupRecordHeader(&output, 1, "workspace", "workspace.json", int64(len(plain)), digest[:]); err != nil { t.Fatal(err) }
	var counter uint64
	if err := writeEncryptedChunks(&output, gcm, prefix, &counter, 1, "workspace", "workspace.json", bytes.NewReader(plain), int64(len(plain)), nil, 0, 1); err != nil { t.Fatal(err) }
...
func TestWorkspaceFormatRestoreRejectsWithoutActivation(t *testing.T) {
	for _, name := range []string{"unknown_top", "unknown_nested", "future_schema"} {
		t.Run(name, func(t *testing.T) {
			...
			switch name {
			case "unknown_top": document["future_extension"] = json.RawMessage(`true`)
			case "unknown_nested": document["settings"] = json.RawMessage(`{"future_option":true}`)
			case "future_schema": document["schema"] = json.RawMessage(fmt.Sprint(Schema + 1))
			}
			...
			before := workspaceFormatTree(t, root)
			view := v.Snapshot()
			if _, err := v.RestorePortableBackup(path, passphrase, nil); err == nil { t.Error("incompatible backup manifest was activated") }
			if !reflect.DeepEqual(view, v.Snapshot()) || !reflect.DeepEqual(before, workspaceFormatTree(t, root)) { t.Error("active workspace changed after incompatible restore") }
			for _, pattern := range []string{root + ".restore-*", root + ".pre-restore-*"} {
				matches, err := filepath.Glob(pattern)
				if err != nil { t.Fatal(err) }
				if len(matches) != 0 { t.Errorf("restore left unexpected workspace directories: %v", matches) }
			}
		})
	}
}
...
func TestWorkspaceFormatCompatibleBackupFixture(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	defer v.Close()
	if _, err := v.RestorePortableBackup(path, passphrase, nil); err != nil { t.Fatal(err) }
	got := v.Snapshot()
	if len(got.Matters) != 1 || got.Matters[0].Title != "Synthetic retained matter" || !got.Settings.LowSensory { t.Fatal("compatible restore lost synthetic records") }
}
