	...
	f.active.restoreBoundary = func(phase string) {
		...
		for _, entry := range entries {
			...
			if path != f.stageRoot && strings.HasPrefix(entry.Name(), filepath.Base(f.root)+".restore-") {
				if stageRoot != "" {
					t.Fatal("ambiguous synthetic restore stage")
				}
				stageRoot = path
			}
		}
		...
		if err := os.Rename(stageRoot, stageRoot+".retained"); err != nil {
			t.Fatal(err)
		}
		if err := os.Mkdir(stageRoot, 0700); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(stageRoot, "unrelated.txt"), []byte("retain synthetic replacement"), 0600); err != nil {
			t.Fatal(err)
		}
	}
	_, err := f.active.RestorePortableBackup(backup, forcedRestorePassphrase, nil)
	if !errors.Is(err, ErrVaultClosed) {
		t.Fatalf("report lost initiating failure: %v", err)
	}
	for _, text := range []string{"cleanup", strconv.Quote(stageRoot), "Open existing"} {
		if !strings.Contains(err.Error(), text) {
			t.Errorf("returned failure omits %q", text)
		}
	}
	if !reflect.DeepEqual(before, workspaceFormatTree(t, f.root)) {
		t.Error("reporting path changed the original workspace")
	}
	if _, err := os.Stat(filepath.Join(stageRoot, "unrelated.txt")); err != nil {
		t.Fatal("cleanup removed unrelated replacement")
	}
	if err := ValidateExistingWorkspaceRoot(stageRoot + ".retained"); err != nil {
		t.Fatal("displaced synthetic stage was not retained")
	}
}
