...
func TestRestoreForcedStopRestart(t *testing.T) {
	for _, phase := range []string{"stage_verified", "original_renamed", "original_retargeted", "staged_renamed", "activated"} {
		t.Run(phase, func(t *testing.T) {
			...
			if gap {
				if v != nil { _ = v.Close() }
				if err == nil { t.Error("restart silently created fresh state during interrupted restore") }
				...
				want := replacement
				if phase == "stage_verified" { want = original }
			...
			if phase != "stage_verified" {
				...
				for _, entry := range entries {
					if strings.HasPrefix(entry.Name(), filepath.Base(f.root)+".pre-restore-") {
						if checkpoint != "" { t.Fatal("ambiguous test checkpoint") }
						checkpoint = filepath.Join(filepath.Dir(f.root), entry.Name())
					}
				}
				if checkpoint == "" { t.Fatal("forced restore lost the original checkpoint") }
...
func verifyForcedRestoreView(t *testing.T, v *Vault, want Workspace) {
	...
	if err := v.Save(); err != nil { t.Fatalf("ownership/CAS did not recover after process exit: %v", err) }
}
...
func killRestoreAtBoundary(t *testing.T, phase, root, backup, marker string) {
	...
	waited := false
	defer func() {
		if !waited { _ = cmd.Process.Kill(); _ = cmd.Wait() }
	}()
	...
	select {
	case err := <-decoded:
		if err != nil { t.Fatalf("child did not reach %s: %v", phase, err) }
	case <-ctx.Done():
		t.Fatalf("child timed out before controlled stop at %s", phase)
	}
	if reached.Phase != phase { t.Fatalf("wrong child boundary: %q", reached.Phase) }
...
func TestRestoreForcedStopChild(t *testing.T) {
	if os.Getenv("ECO_TEST_FORCED_RESTORE") != "1" { return }
	...
	v.restoreBoundary = func(phase string) {
		if phase != os.Getenv("ECO_TEST_RESTORE_PHASE") { return }
		if err := json.NewEncoder(os.Stdout).Encode(struct{ Phase string }{phase}); err != nil { t.Fatal(err) }
		var one [1]byte
